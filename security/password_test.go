...
func TestPasswordManagerRejectsMalformedHashesBeforeArgon2(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "carriage return in salt", encoded: "$argon2id$v=19$m=19456,t=1,p=1$" + validSalt[:1] + "\r" + validSalt[1:] + "$" + validHash},
		{name: "line feed in salt", encoded: "$argon2id$v=19$m=19456,t=1,p=1$" + validSalt[:1] + "\n" + validSalt[1:] + "$" + validHash},
		...
		{name: "carriage return in hash", encoded: "$argon2id$v=19$m=19456,t=1,p=1$" + validSalt + "$" + validHash[:1] + "\r" + validHash[1:]},
		{name: "line feed in hash", encoded: "$argon2id$v=19$m=19456,t=1,p=1$" + validSalt + "$" + validHash[:1] + "\n" + validHash[1:]},
